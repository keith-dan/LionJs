...
Element.prototype.onserialize = function (key, value) {
                        ...
                        if (_selectedElement[i].editMode > namespace.EditMode.Select) {
                            ...
                            if (dragEle && _stepMove) {
                                ...
                                if (Math.abs(e.offsetX - _offset.x) > Math.abs(e.offsetY - _offset.y)) {
                                    if (e.offsetX - _stepOffset.x > _stepGrid.x * _scale) {
                                        offsetX = _stepGrid.x;
                                        _stepOffset.x = e.offsetX;
                                    }

                                    else if (e.offsetX - _stepOffset.x < (0 - _stepGrid.x) * _scale) {
                                        offsetX = (0 - _stepGrid.x);
                                        _stepOffset.x = e.offsetX;
                                    }


                                } else {
                                    if (e.offsetY - _stepOffset.y > _stepGrid.y * _scale) {
                                        offsetY = _stepGrid.y;
                                        _stepOffset.y = e.offsetY;
                                    } else if (e.offsetY - _stepOffset.y < (0 - _stepGrid.y) * _scale) {

                                        offsetY = (0 - _stepGrid.y);
                                        _stepOffset.y = e.offsetY;
                                    }
    ...
    !(function (namespace) {
        ...
        */
        ...
        MixElement.prototype.setScale = function (scale) {
            for (var i = 0; i < this.elements.length; i++) {
                this.elements[i].setScale(scale);
            }
            namespace.RectElement.prototype.setScale.call(this, scale);
        }
